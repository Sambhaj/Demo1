...
include('Config.php'); // // include database file
	...
	{  
		...
		if (!empty($_POST["email"]))
		{
			...
			{
				$incrtmail = "Incorrect email format!";
			}
			else
			{
			if(isset($_POST["email"]))
			{
				// Query for check email from table
				$query = mysql_query("SELECT * FROM admin WHERE Email_ID = '$_POST[email]'") or die(mysql_error());
				$exist = mysql_num_rows($query);
			
				if($exist >=1)
				{
					$query_parent = mysql_query("SELECT * FROM admin WHERE Email_ID = '$_POST[email]'") or die("Query failed: ".mysql_error());
					$r=mysql_fetch_row($query_parent);	
				
				// send mail on email
					$formcontent=" From: THE MAHARASHTRA BELTS AND HOSES TRADERS ASSOCIATION \n Username: $r[2] \n Password: $r[3]";
					$recipient = "$email";
					$subject = "MBHTA Admin Login Details";
					$mailheader = "From: $email \r\n";
					mail($recipient, $subject, $formcontent, $mailheader) or die("Error!");
					
					$mailsend="Password send to your email address";
				}
				else
				{
					$mailnotmatch="Please provide correct email address!";
				}
			}
			}
		}
...
<html lang="en-US">
    ...
    <head>
        ...
        <title>Forget Password | THE MAHARASHTRA BELTS &amp; HOSES TRADERS ASSOCIATION</title>
...
<link href='https://fonts.googleapis.com/css?family=Exo+2:400,700' rel='stylesheet' type='text/css'>
	...
	<div style="height:35px;">
		<p style="color:white;margin-top:5px;padding:5px;text-align:center;font-size:18px"><b>Forget Password
		</b></p>
	</div>
<!-------------End Admin Header---------------------->  
...
<!-------------Your contents body---------------------->			
	<a href="Admin_Login.php"><img src="images/close.png" style="float:right;width:40px;height:40px;border:1px solid #D5D5D5;"></a>
		
	<center>
		<form method="POST" action="">
		<table>
		<tr>
		<td><center><input type="text" style="width:40%;" name="email" placeholder="Enter your email address">
		<input type="submit" name="submit"></center></td>
		</tr>
		<tr>
		<td><center>
		<span style="color:red;"><?php echo $incrtmail;?></span>
		<span style="color:red;"><?php echo "<b style='color:green;'>"; echo $mailsend; echo "</b>";?></span>
		<span style="color:red;"><?php echo $mailnotmatch;?></span>
		<span style="color:red;"><?php echo $blank;?></span></center>
		</td>
		</tr>
		</table>
		</form>
