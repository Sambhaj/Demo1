...
if(isset($_POST['submit']))
	{function GetImageExtension($imagetype)
		...
		$res1 = mysql_query("SELECT max(Circular_ID) FROM  circulars");
		...
		if(is_null($res2[0]))
		{
			...
			if (!empty($_FILES["uploaddoc"]["name"])&&!empty($_POST["head"])&&!empty($_POST["desc"]))
			{
				// for data upload
				$head = $_POST['head']; 
				$desc = $_POST['desc']; 
				...
				$imageget=$maxvalue."_".$imagename;
				$target_path = "upload/doc/".$imageget;
			
				if(move_uploaded_file($temp_name, $target_path)) 
				{
					mysql_query("INSERT into circulars (Circular_ID,Headlines,Details,Upload_Doc) values('$maxvalue','$head','$desc','$target_path') ") or die(mysql_error());
					
					print '<script>alert("Data Successfully Uploaded");</script>' ; // for successful data upload
					Print '<script>window.location.assign("Circulars.php");</script>';
				}
			}
			else
			{
...
<html lang="en-US">
    ...
    <head>
        ...
        <title>Circulars | THE MAHARASHTRA BELTS &amp; HOSES TRADERS ASSOCIATION</title>
...
<!-------------Your contents body---------------------->	
		...
		<li class='active'><a href='Circulars.php'><span>Circulars</span></a></li>
		...
		<table>
		<tr>
		<td colspan="2" align="center" style="border-bottom:1px #272727 solid; text-align:center;font-size:16px;padding:10px;background-color:#38A3E1;font-family: 'roboto';color:#FFFFFF;">Add Circular
		</td>
		</tr>
		<tr>
		<td style="text-align:right;">Heading:</td>
		<td>
		<input style="width:60%;" type="text" name="head" value="<?php echo isset($_POST['head']) ? $_POST['head'] : ''; ?>"><span style="color:red;"><?php echo $msg;?></span>
		</td>
		</tr>
		<tr>
		<td style="text-align:right;">Description:</td>
		<td>
		<textarea style="width:60%;" rows="7" name="desc" ><?php echo isset($_POST['desc']) ? $_POST['desc'] : ''; ?></textarea><span style="color:red;"><?php echo $msg;?></span>
		</td>
		</tr>
		<tr>
		<td style="text-align:right;">Upload Doc:</td>
		<td>
		<span style="color:red;"><?php echo $msg;?></span><input name="uploaddoc" type="file">
		</td>
		</tr>
		<tr>
		<td>
		...
		<table>
		<tr>
		<td style="border-bottom:1px #272727 solid;width:60px; text-align:center;font-size:16px;padding:10px;background-color:#38A3E1;font-family: 'roboto';color:#FFFFFF;">ID
		</td>
		<td style="border-bottom:1px #272727 solid; width:95%;text-align:center;font-size:16px;padding:10px;background-color:#38A3E1;font-family: 'roboto';color:#FFFFFF;">Circulars
		</td>
		</tr>
		</table>
		<object type='text/html' data='Update_Remove_Circulars.php' width='100%' height='400' style='overflow: scroll;'></object>
