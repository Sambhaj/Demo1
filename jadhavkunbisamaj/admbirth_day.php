...
{
$qr="Select Max(news_id) as id from tbbirth_death";
...
//updimgs("img");                          //here called function to upload imgae to server folder
 ...
 $newname = "birthday_" . $memberid . "." .$ext; 
 ...
 // echo "<script> alert('File not moved to destination folder. Check permissions')       //  Echo "File not moved to destination folder. Check permissions";
   ...
   // echo "<script> alert('File is not uploaded.')  Echo "File is not uploaded.";
	...
	if(empty($newsid) || empty($headline) || empty($news) ||  empty($pdate) || empty($submitby) || empty($birthdate))
	{
	...
	$qry="insert into tbbirth_death values('$newsid','$headline','$news','$newname','$pdate','$submitby','$birthdate')";	
...
//echo  '<META http-equiv="refresh" content="0;URL=admbirth_day.php">';           
}
else
{
...
<form action="admbirth_day.php" method="post" enctype="multipart/form-data">
<table>
</tr>
...
<tr><td>Image</td><td><input type="file" name="img" value="<?php echo $newname;?>" /></td></tr>
<tr><td>Create Date</td><td> <input type="text" name="pdate" id="pd"  value="<?php echo $pdate;?>" size=10/></td></tr>
<tr><td>Submitted By</td><td><input type="text" name="submittedby" value="<?php echo $submitby;?>"/></td></tr>
<tr><td>Birth Day</td><td> <input type="radio" value="Yes" name="Yes" <?php if (isset($birthdate) && $birthdate=="1") echo "checked";?>  />Yes<input type="radio" value="No" name="Yes" <?php if (isset($birthdate) && $birthdate=="0") echo "checked";?> />No   </td></tr>
