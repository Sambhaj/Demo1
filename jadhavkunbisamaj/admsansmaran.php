...
{
$qr="Select Max(smid) as id from tbsansmaran";
...
//updimgs("img");                          //here called function to upload imgae to server folder
 ...
 $newname = "sansmaran_" . $memberid . "." .$ext; 
 ...
 // echo "<script> alert('File not moved to destination folder. Check permissions')       //  Echo "File not moved to destination folder. Check permissions";
   ...
   // echo "<script> alert('File is not uploaded.')  Echo "File is not uploaded.";
	...
	if(empty($newsid) || empty($headline) || empty($pdate) || empty($submitby))
	{
	...
	$qry="insert into tbsansmaran values('$newsid','$headline','$newname','$pdate','$submitby')";	
...
<form action="admsansmaran.php" method="post" enctype="multipart/form-data">
<table>
</tr>
<tr><td>Sansmaran Id</td><td><input type="text" name="newsid" value="<?php echo $idd;?>" /></td></tr>
<tr><td>Sansmaran Text</td><td><input type="text" name="headline" value="<?php echo $headline;?>"/></td></tr>
